...
if (!html.includes("Copyright (c) 2026 Haichao Li")) throw new Error("prototype missing embedded reference-project license");
if (!html.includes("Copyright © 2010-2026 three.js authors")) throw new Error("prototype missing embedded Three.js license");
if (/<script[^>]+src=|<link[^>]+href=|<(?:img|video|audio|source)\b[^>]+src=/i.test(html)) throw new Error("prototype contains an external runtime resource tag");
if (Buffer.byteLength(html) > 750 * 1024) throw new Error("prototype exceeds the 750 KiB offline budget");
