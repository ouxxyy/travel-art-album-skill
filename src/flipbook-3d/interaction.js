...
export function edgePreviewDirection({
  ...
  if (sheet <= 0)
    return Math.abs(pointerX - (centerX + pageWidth / 2)) <= edgeZone ? 1 : 0;
  if (sheet >= sheetCount)
    return Math.abs(pointerX - (centerX - pageWidth)) <= edgeZone ? -1 : 0;
